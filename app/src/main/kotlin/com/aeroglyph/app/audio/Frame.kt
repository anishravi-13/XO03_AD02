...
/** The 2-bit frame-type flag packed into the header's third byte alongside hop count. */
enum class FrameType(val bits: Int) {
    ...
    companion object {
        fun fromBits(bits: Int): FrameType = entries.first { it.bits == (bits and 0b11) }
    }
}
 ...
 * byte 2        (frameType << 6) | hopCount   -- hopCount fits in 6 bits (0-63)
...
data class Frame(
    ...
    init {
        ...
        require(hopCount in 0..63) { "hopCount out of range: $hopCount" }
    ...
    fun toPlaintextBytes(): ByteArray {
        val header = byteArrayOf(
            payload.size.toByte(),
            sessionId.toByte(),
            (((frameType.bits and 0b11) shl 6) or (hopCount and 0x3F)).toByte(),
        )
    ...
    companion object {
        ...
        fun parseTypeAndHop(byte3: Int): Pair<FrameType, Int> {
            val frameType = FrameType.fromBits((byte3 shr 6) and 0b11)
            val hopCount = byte3 and 0x3F
